...
const LoginFunctions = {
    ...
    registerOrLink: async function (provider, credential,
                                    email,
                                    fullName,
                                    address,
                                    date,
        ...
        this.saveCredential(provider, credential)
        let user = await auth().createUserWithEmailAndPassword(credential.token, credential.secret)
            .then((res) => {
                    ...
                    firebase
                        ...
                        .then(() => {
                            ...
                            if(userPhoto) {
                                ...
                                const task =  reference.putFile(Platform.OS == 'android' ? userPhoto.uri : userPhoto.sourceURL);
    ...
    getPhotoUrl: async function (userId) {
        const url = await  storage().ref(  userId + '/profile-image.png').getDownloadURL()

        return url
    },


    registerOrLinkTrainer: async function (provider, credential,
                                    email,
                                    fullName,
                                    address,
                                    date,
